  ...
  makeStore(Bottle, 'bottles'),
  {
    async preSave(data) {
      ...
      if (data.image instanceof File) {
        ...
        await storageApi.child(path).put(data.image)
        data.image = path
      }
    ...
    async updateFromLogs(bottleRefs) {
      const collectionRef = await logStore.collection()
      const batch = await this.batch()

      const promises = bottleRefs.map(async bottleRef => {
        // First fetch bottle logs
        const snapshot = await logStore
...
export function useBottles(visibility) {
  const name = undefined === visibility ? 'all' : visibility ? 'stocked' : 'unstocked'
